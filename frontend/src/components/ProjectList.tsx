...
import { deleteProject, getUserRoleFromToken } from "../api/api";
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {projects.map((project) => {
        ...
        const canDelete = isManager || globalRole === "ADMIN";
        ...
        return (
          ...
          >
            ...
            {(isManager || globalRole === "ADMIN") && (
              <div
                className={`absolute bottom-2 right-2 text-xs font-semibold px-2 py-1 rounded shadow-sm ${
                  globalRole === "ADMIN"
                    ? "bg-red-100 text-red-700"
                    : "bg-green-100 text-green-700"
                }`}
              >
                {globalRole === "ADMIN" ? "ADMIN" : "MANAGER"}
              </div>
            )}
